...
data FuseOperations fh = FuseOperations
      { -- | Implements 'System.Posix.Files.getSymbolicLinkStatus' operation
        ...
        --   flags.
        fuseOpen :: FilePath -> OpenMode -> OpenFileFlags -> IO (Either Errno (FuseOpenInfo, fh)),
    ...
    where fuseHandler :: e -> IO CInt
          ...
          wrapOpen pFilePath pFuseFileInfo = handle fuseHandler $
              do filePath <- peekCString pFilePath
                 ...
                 case result of
                    Left (Errno errno)     -> return (- errno)
                    Right (openInfo, cval) -> do
                        sptr <- newStablePtr cval
                        (#poke struct fuse_file_info, fh) pFuseFileInfo $ castStablePtrToPtr sptr
                        (#poke_bit struct fuse_file_info, direct_io) pFuseFileInfo $ toInt $ fsDirectIo openInfo
                        (#poke_bit struct fuse_file_info, keep_cache) pFuseFileInfo $ toInt $ fsKeepCache openInfo
                        (#poke_bit struct fuse_file_info, nonseekable) pFuseFileInfo $ toInt $ fsNonseekable openInfo
                        return okErrno
             where
	        toInt :: Bool -> CUInt
	        toInt True = 1
...
data FuseOpenInfo = FuseOpenInfo {
      fsDirectIo :: Bool
    , fsKeepCache :: Bool
    , fsNonseekable :: Bool
    }
...
foreign import ccall safe "fuse.h fuse_interrupted"
    fuse_interrupted :: IO (CInt)
...
-- from http://www.mail-archive.com/haskell-cafe@haskell.org/msg48476.html
